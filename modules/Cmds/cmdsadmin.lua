...
function AdminRevivePlayer(playerid, params)
	local result, id = sscanf(params, "d");
	if result == 1 then
		if IsDead(id) == 1 or Player[id].dead == 1 then
			...
			SSM(playerid, "Вы возродили "..GetPlayerName(id)..".");
			SSM(id, "Вы были возрождены "..GetPlayerName(playerid)..".");
			Player[id].dead = 0;
			LogString("Logs/Admins/heal", GetPlayerName(playerid).." исцеляет "..GetPlayerName(id));
		else
			SEM(playerid, "Указанный вами игрок не мертв, используйте /хилл.");
		end
	else
		SYNM(playerid, "/рес (ид)");
	end
end
addCommandHandler({"/оживить", "/рес"}, AdminRevivePlayer);

function AdminAnimation(playerid, params)
	local result, anim = sscanf(params, "s");
	if result == 1 then
		SSM(playerid, "Вы воспроизвели анимацию "..GetPlayerAnimationName(playerid).." ("..GetPlayerAnimationID(playerid)..")");
		PlayAnimation(playerid, anim);
	
	else
		SYNM(playerid, "/таним (название)");
	end
end
addCommandHandler({"/таним"}, AdminAnimation);